...
================================
Structured analyst context for common Linux log events detected by Wazuh.

Unlike Windows Event IDs, Linux events are identified by semantic keys
based on program / source / log patterns rather than numeric IDs.

Key format:  linux.<subsystem>.<event_type>
Example:     linux.ssh.login_failure

Knowledge levels
----------------
  deep    - Full analyst context entry in this KB
  pattern - Matched by pattern classification (no deep entry)
  basic   - Matched using Wazuh rule description
...
---------
  ...
  - systemd unit created / modified / enabled / disabled
...
# ---------------------------------------------------------------------------
# Legacy key normaliser  (old flat name → new namespaced key)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Sensitive FIM paths — escalate severity when file_path matches one of these
# ---------------------------------------------------------------------------
SENSITIVE_FIM_PATHS: tuple[str, ...] = (
    "/etc/passwd",
    "/etc/shadow",
    "/etc/sudoers",
    "/etc/sudoers.d/",
    ...
    "/home/",           # catches /home/*/.ssh/authorized_keys
    "/etc/systemd/system/",
    "/etc/cron.d/",
    "/etc/crontab",
    "/var/spool/cron/",
...
# Knowledge base
# ---------------------------------------------------------------------------
LINUX_EVENT_KNOWLEDGE: dict[str, dict] = {
    ...
    "linux.ssh.login_failure": {
        ...
        "summary": (
            "An SSH login attempt failed due to bad credentials, "
            "an invalid username, or a key mismatch."
        ),
        "what_triggers_it": [
        ...
        "important_fields": ["user", "src_ip", "src_port", "auth_method"],
        "field_interpretation": {
            "user": (
                "Attempted username — 'root', 'admin', 'guest' are high-signal "
                "if this host uses key-only auth"
            ),
            "src_ip": "Source IP of the attacker or misconfigured client",
            "src_port": "Ephemeral port from the client — changes each attempt",
            "auth_method": (
                "password / publickey / gssapi — password attempts are higher risk"
            ),
        },
    ...
    "linux.ssh.login_success": {
        ...
        "summary": (
            "An SSH login was accepted using password or "
        ...
        "important_fields": ["user", "src_ip", "src_port", "auth_method", "key_fingerprint"],
        "field_interpretation": {
            "user": "Authenticated username",
            "src_ip": "Source IP — compare to known baselines",
            "auth_method": "'publickey' is generally safer than 'password'",
            "key_fingerprint": (
                "For key auth — track key fingerprints to detect new/unknown keys"
            ),
        },
        "common_benign_causes": [
    ...
    "linux.local.login_success": {
        ...
        "escalation_conditions": [
            "Console login on cloud/virtual server (physical hypervisor access)",
            "Console login by root",
            "Console login by unknown user account",
        ],
        "baseline_conditions": [
            "Known admin account, documented maintenance window",
        ],
        "false_positive_notes": [
    ...
    "linux.sudo.command": {
        ...
        "default_severity": "medium",
        "summary": (
            "A user successfully executed a command with elevated "
        ...
        "important_fields": ["user", "tty", "pwd", "target_user", "command"],
        "field_interpretation": {
    ...
    "linux.sudo.command_failure": {
        ...
        "summary": (
            "A sudo attempt was denied — wrong password, not in sudoers, "
            "or PAM authentication failure."
        ),
        "what_triggers_it": [
        ...
        "important_fields": ["user", "tty", "command", "reason"],
        "field_interpretation": {
            "user": "User who attempted sudo",
            "tty": "Terminal source",
            "command": "Command that was attempted — important for intent analysis",
            "reason": (
                "user NOT in sudoers / authentication failure / "
                "3 incorrect password attempts"
            ),
        },
        "common_benign_causes": [
            "New employee forgot their account does not have sudo rights yet",
        ...
        "recommended_checks": [
            "Verify user's sudo authorisation in /etc/sudoers and /etc/sudoers.d/",
        ...
        "baseline_conditions": [
            "Single failure by a new employee who forgot their rights",
        ],
        "false_positive_notes": [
    ...
    "linux.cron.execution": {
        ...
        "important_fields": ["user", "command", "cron_pid"],
        "field_interpretation": {
    ...
    "linux.package.installed": {
        ...
        "important_fields": ["package_name", "version", "action", "user"],
        "field_interpretation": {
            "package_name": (
                "Installed package — network tools, compilers, RATs are high-signal"
            ),
        ...
        "high_signal_patterns": [
            ...
            "Python, perl, ruby interpreter added to a server with no development role",
        ],
        "related_events": [
            "linux.sudo.command",
    ...
    "linux.package.removed": {
        ...
        "important_fields": ["package_name", "version", "action"],
        "field_interpretation": {
            "package_name": (
                "Removed package — security tools being removed is especially suspicious"
            ),
        ...
        "baseline_conditions": [
            "Package cleanup as part of documented decommission",
        ],
        "false_positive_notes": [
    ...
    "linux.kernel.oops": {
        ...
        "default_severity": "high",
        "summary": (
            "The Linux kernel encountered a non-fatal error (oops) — "
            "a BUG condition or invalid memory access."
        ),
        "what_triggers_it": [
        ...
        "important_fields": ["fault_type", "address", "call_trace", "module"],
        "field_interpretation": {
    ...
    "linux.kernel.panic": {
        ...
        "important_fields": ["panic_reason", "call_trace"],
        "field_interpretation": {
        ...
        "baseline_conditions": [
            "Single panic during kernel/module upgrade (expected)",
        ],
        "false_positive_notes": [
            "Kernel panics are always serious — no routine false positives",
        ],
        "references": [
    ...
    "linux.firewall.ufw_block": {
        ...
        "important_fields": ["src_ip", "dst_ip", "src_port", "dst_port", "protocol", "interface"],
        "field_interpretation": {
        ...
        "baseline_conditions": [
            "Random internet noise on common ports: 22, 80, 443, 8080",
        ],
        "false_positive_notes": [
    ...
    "linux.fim.file_modified": {
        ...
        "default_severity": "medium",
        "summary": (
            "Wazuh File Integrity Monitoring detected a modification "
            "to a monitored file."
        ),
        "what_triggers_it": [
        ...
        "important_fields": ["file_path", "old_hash", "new_hash", "permissions", "owner", "group"],
        "field_interpretation": {
...
def normalize_linux_event_key(key: str) -> str:
    """
    Normalise a legacy flat key or a namespaced key to a canonical
    ``linux.<subsystem>.<event>`` key.

    Examples::

        normalize_linux_event_key("ssh_failed_login")       == "linux.ssh.login_failure"
        normalize_linux_event_key("linux.ssh.login_failure") == "linux.ssh.login_failure"
        normalize_linux_event_key("unknown_key")             == "unknown_key"
    """
    return _LEGACY_KEY_MAP.get(key, key)


def get_linux_event_knowledge(key: str) -> dict | None:
    """Return the full knowledge entry for *key*, or ``None`` if not found.

    Accepts both legacy flat keys and namespaced keys.
    """
    canon = normalize_linux_event_key(key)
    return LINUX_EVENT_KNOWLEDGE.get(canon)


def get_linux_event_summary(key: str) -> str:
    """Return the one-line summary for *key*, or an empty string."""
    entry = get_linux_event_knowledge(key)
    return entry["summary"] if entry else ""


def get_linux_recommended_checks(key: str) -> list[str]:
    """Return the recommended_checks list for *key*, or an empty list."""
    entry = get_linux_event_knowledge(key)
    return list(entry["recommended_checks"]) if entry else []


def get_linux_related_events(key: str) -> list[str]:
    """Return the related_events list for *key*, or an empty list."""
    entry = get_linux_event_knowledge(key)
    return list(entry["related_events"]) if entry else []


# ---------------------------------------------------------------------------
# Pattern-based resolver
# ---------------------------------------------------------------------------
...
def _make_generic_response(
    ...
    knowledge_level: str = "pattern",
) -> dict:
    """Build a minimal response object for events not in the deep KB."""
    return {
        "key": key,
        "title": title,
        "category": category,
...
def resolve_linux_event_from_log(
    program: str | None,
    message: str | None,
    source: str | None = None,
    ...
    """
    Classify a Linux log event and return a knowledge response.

    Resolution priority
    -------------------
    1. Deep match — entry present in :data:`LINUX_EVENT_KNOWLEDGE`
    2. Pattern match — classify by ``program`` + ``message`` patterns
    3. Basic — use ``wazuh_rule_description`` as summary
    4. Unknown fallback

    Parameters
    ----------
    program:
        The process / program name from the log record (e.g. ``"sshd"``).
    message:
        The raw log message text.
    source:
        Optional log source path (e.g. ``"/var/log/auth.log"``).
    wazuh_rule_description:
        Optional Wazuh rule description string used as fallback.

    Returns
    -------
    dict
        A normalised response dict.  ``knowledge_level`` is one of
        ``"deep" | "pattern" | "basic" | "unknown"``.
    """
    prog = (program or "").lower().strip()
    ...
    # 1. SSH (sshd)
    # ------------------------------------------------------------------
    if prog == "sshd" or "sshd" in msg:
        if re.search(r"(failed password|invalid user|authentication failure)", msg):
            key = "linux.ssh.login_failure"
        elif re.search(r"(accepted password|accepted publickey|accepted keyboard)", msg):
            key = "linux.ssh.login_success"
        else:
            key = None

        if key:
            entry = LINUX_EVENT_KNOWLEDGE[key].copy()
            entry["knowledge_level"] = "deep"
            return entry

    # ------------------------------------------------------------------
    # 2. sudo
    # ------------------------------------------------------------------
    if prog == "sudo" or prog.startswith("sudo"):
        if re.search(r"(not in sudoers|authentication failure|incorrect password attempts)", msg):
            key = "linux.sudo.command_failure"
        elif re.search(r"command=", msg):
            key = "linux.sudo.command"
        else:
            key = None
    ...
    # 3. PAM / local login
    # ------------------------------------------------------------------
    if prog in ("login", "su", "gdm", "lightdm") or "pam_unix" in msg:
        if re.search(r"session opened for user", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.local.login_success"].copy()
            entry["knowledge_level"] = "deep"
            return entry
    ...
    # 4. Cron
    # ------------------------------------------------------------------
    if prog in ("cron", "crond") or "cron" in prog:
        if re.search(r"\bcmd\b", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.cron.execution"].copy()
            entry["knowledge_level"] = "deep"
            return entry

    # ------------------------------------------------------------------
    # 5. Package management (dpkg / apt / yum / dnf / rpm)
    # ------------------------------------------------------------------
    if prog in ("dpkg", "apt", "apt-get", "yum", "dnf", "rpm"):
        if re.search(r"\b(install|upgrade|reinstall)\b", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.package.installed"].copy()
            entry["knowledge_level"] = "deep"
            return entry
        if re.search(r"\b(remove|purge|erase)\b", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.package.removed"].copy()
            entry["knowledge_level"] = "deep"
            return entry

    # ------------------------------------------------------------------
    # 6. Kernel (via dmesg / kern.log)
    # ------------------------------------------------------------------
    if prog == "kernel" or prog == "dmesg":
        if re.search(r"kernel panic|not syncing", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.kernel.panic"].copy()
            entry["knowledge_level"] = "deep"
            return entry
        if re.search(r"\bbug\b|\boops\b|page fault|general protection", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.kernel.oops"].copy()
            entry["knowledge_level"] = "deep"
            return entry
        # UFW is logged as kernel (netfilter)
        if "ufw block" in msg:
            entry = LINUX_EVENT_KNOWLEDGE["linux.firewall.ufw_block"].copy()
            entry["knowledge_level"] = "deep"
            return entry

    # ------------------------------------------------------------------
    # 7. UFW (may come as separate program or raw message)
    # ------------------------------------------------------------------
    if "ufw block" in msg or "[ufw block]" in msg:
        entry = LINUX_EVENT_KNOWLEDGE["linux.firewall.ufw_block"].copy()
        entry["knowledge_level"] = "deep"
        return entry
    ...
    # ------------------------------------------------------------------
    if prog in ("ossec-syscheckd", "wazuh-syscheckd", "syscheck", "wazuh fim") or \
            "syscheck" in msg or "fim" in msg.lower():
        if re.search(r"(modified|checksum changed|has been modified)", msg):
            entry = LINUX_EVENT_KNOWLEDGE["linux.fim.file_modified"].copy()
            # Check if the path is sensitive → escalate severity
            for sensitive_path in SENSITIVE_FIM_PATHS:
                if sensitive_path in raw_msg:
                    entry = entry.copy()
                    entry["default_severity"] = "high"
                    entry["summary"] = (
                        f"FIM detected modification of a sensitive path "
                        f"({sensitive_path.rstrip('/')}): "
                        f"{entry['summary']}"
                    )
                    break
            entry["knowledge_level"] = "deep"
    ...
    # ------------------------------------------------------------------
    # 9. Basic fallback — use Wazuh rule description
    # ------------------------------------------------------------------
    if wazuh_rule_description:
...
if __name__ == "__main__":
    ...
    print("Linux Event KB  v0.1  — sanity tests\n")

    _TESTS: list[tuple[str, str, str, str]] = [
        (
            "sshd",
            "Failed password for root from 1.2.3.4 port 45182 ssh2",
            "linux.ssh.login_failure",
        ...
        (
            "sshd",
            "Accepted publickey for colin from 172.21.1.20 port 53322 ssh2",
            "linux.ssh.login_success",
        ...
        (
            "kernel",
            "[UFW BLOCK] IN=eth0 SRC=77.72.85.26 DST=157.230.26.180 PROTO=TCP DPT=3194",
            "linux.firewall.ufw_block",
            "UFW block",
        ),
        (
            "Wazuh FIM",
            "syscheck: File '/etc/sudoers' has been modified",
            "linux.fim.file_modified",
            "FIM sensitive file (/etc/sudoers)",
        ),
    ]

    passed = 0
    for prog, msg, expected_key, label in _TESTS:
        result = resolve_linux_event_from_log(program=prog, message=msg)
        ...
        print(
            ...
            f"         severity={result.get('default_severity')!r}  "
            f"level={result.get('knowledge_level')!r}\n"
        )

    # Sensitive path escalation check
    fim_result = resolve_linux_event_from_log(
        program="syscheck",
        message="syscheck: File '/etc/sudoers' has been modified",
    )
    sev_ok = fim_result.get("default_severity") in ("high", "critical")
    status = "PASS" if sev_ok else "FAIL"
    if sev_ok:
        passed += 1
    print(
        f"  [{status}] FIM sensitive path severity escalation\n"
        f"         severity={fim_result.get('default_severity')!r}  "
