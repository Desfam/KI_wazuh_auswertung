...
type LayoutCluster = EventCluster & {
  x: number;
  y: number;
  radius: number;
  zone: 'safe' | 'medium' | 'high' | 'critical' | 'center';
};
...
function zoneForSeverity(severity: Severity): LayoutCluster['zone'] {
...
function layoutClusters(clusters: EventCluster[]): LayoutCluster[] {
  const zones: Record<LayoutCluster['zone'], { x: number; y: number; spreadX: number; spreadY: number; index: number }> = {
    safe: { x: 18, y: 23, spreadX: 18, spreadY: 18, index: 0 },
    medium: { x: 46, y: 20, spreadX: 20, spreadY: 18, index: 0 },
    high: { x: 76, y: 23, spreadX: 20, spreadY: 18, index: 0 },
    critical: { x: 77, y: 64, spreadX: 20, spreadY: 20, index: 0 },
    center: { x: 38, y: 48, spreadX: 22, spreadY: 22, index: 0 },
  };
  ...
  return clusters.map((cluster) => {
    ...
    const offsetX = Math.cos(angle) * z.spreadX * 0.12 * ring + ((i % 2) - 0.5) * 3;
    const offsetY = Math.sin(angle) * z.spreadY * 0.12 * ring + ((i % 3) - 1) * 2;

    return {
      ...cluster,
      zone,
      x: Math.max(7, Math.min(93, z.x + offsetX)),
      y: Math.max(10, Math.min(82, z.y + offsetY)),
      radius: Math.max(18, Math.min(48, 16 + Math.sqrt(cluster.alertCount) * 1.4)),
    };
  });
}
...
function FloatingToolbar() {
  return (
    <div className="waia-floating-tools">
      {['\u25ce', '\u232f', '\ud83d\udee1', '\u2315', '\u26d3', '\u25a4', '\u2699'].map((item) => (
...
function RadarMap({
  ...
  setSelected: (cluster: EventCluster | null) => void;
}) {
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    setTf((prev) => {
      ...
      return { scale: newScale, ox: mx - (mx - prev.ox) * ratio, oy: my - (my - prev.oy) * ratio };
    });
  }, []);
  ...
  const onMouseDown = (e: React.MouseEvent) => {
    ...
    dragRef.current = { startX: e.clientX, startY: e.clientY, ox: tf.ox, oy: tf.oy };
  };
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    setTf((prev) => ({ ...prev, ox: d.ox + dx, oy: d.oy + dy }));
  };

  const onMouseUp = () => { dragRef.current = null; };

  const center = clusters.find((c) => c.title.includes('4625')) ?? clusters[0] ?? null;
  const safeClusters = clusters.filter((c) => c.zone === 'safe');
  const mediumClusters = clusters.filter((c) => c.zone === 'medium');
  const highClusters = clusters.filter((c) => c.zone === 'high');
  const criticalClusters = clusters.filter((c) => c.zone === 'critical');

  return (
    <div
      className="waia-radar"
      ref={containerRef}
    ...
    >
      <div className="waia-matrix-noise" />

      <div
        className="waia-radar-zoomable"
        style={{ transform: `translate(${tf.ox}px,${tf.oy}px) scale(${tf.scale})`, transformOrigin: '0 0' }}
      >
        <svg className="waia-radar-svg" viewBox="0 0 1000 620" preserveAspectRatio="none">
          <defs>
            <filter id="glow-cyan">
            ...
            clusters
              ...
              .map((c) => (
                <line
                  ...
                  strokeWidth={Math.max(0.7, Math.min(3.5, Math.sqrt(c.alertCount) / 7))}
                  opacity="0.38"
                />
              ))}

          {[...safeClusters, ...mediumClusters, ...highClusters, ...criticalClusters].map((cluster) => (
            <ClusterNode
              key={cluster.id}
              cluster={cluster}
              selected={selected?.id === cluster.id}
              onClick={() => { if (!didDrag.current) setSelected(cluster); }}
            />
          ))}

          {center && (
            <g className="waia-center-burst" transform={`translate(${center.x * 10} ${center.y * 6.2})`}>
              <circle r="44" fill="rgba(0,217,255,0.12)" stroke="#00d9ff" strokeWidth="1.6" />
              <circle r="24" fill="rgba(0,217,255,0.28)" stroke="#e2ffff" strokeWidth="2" />
              <circle r="9" fill="#e2ffff" />
            </g>
          )}
        </svg>
        ...
        {clusters.map((cluster) => (
          <button
            key={cluster.id}
            type="button"
            className={cx('waia-event-label', selected?.id === cluster.id && 'selected')}
            style={{ left: `${cluster.x}%`, top: `${cluster.y}%`, '--event-color': sevColor(cluster.severity) } as CSSProperties}
            onClick={(e) => { e.stopPropagation(); if (!didDrag.current) setSelected(cluster); }}
          >
            {cluster.title}
          </button>
        ))}
      ...
      {selected && <EventPopup cluster={selected} onClose={() => setSelected(null)} />}

      <FloatingToolbar />

      {tf.scale !== 1 && (
        <button
          className="waia-zoom-reset"
          onClick={() => setTf({ scale: 1, ox: 0, oy: 0 })}
        >
          Reset zoom
        </button>
      )}
...
function ClusterNode({
  ...
  const color = sevColor(cluster.severity);
  const nodeCount = Math.min(22, Math.max(6, Math.round(Math.sqrt(cluster.alertCount))));

  const children = Array.from({ length: nodeCount }).map((_, index) => {
    const angle = (Math.PI * 2 * index) / nodeCount + index * 0.37;
    const distance = cluster.radius * (0.45 + (index % 4) * 0.15);
  ...
  return (
    <g
      className={cx('waia-cluster-node', selected && 'selected')}
      transform={`translate(${cluster.x * 10} ${cluster.y * 6.2})`}
      onClick={onClick}
      style={{ cursor: 'pointer' }}
    >
      <circle
        r={cluster.radius * 1.7}
        fill={color}
        opacity="0.07"
        filter={cluster.severity === 'critical' || cluster.severity === 'high' ? 'url(#glow-strong)' : 'url(#glow-cyan)'}
      />
      <circle r={cluster.radius * 0.32} fill={color} opacity="0.85" filter="url(#glow-cyan)" />
      ...
      <circle r={cluster.radius * 0.46} fill="transparent" stroke={color} strokeWidth="1.6" opacity="0.95" />
    </g>
  );
}

function EventPopup({ cluster, onClose }: { cluster: EventCluster; onClose: () => void }) {
  const color = sevColor(cluster.severity);
  const topHost = cluster.affectedHosts[0];

  return (
    <div className="waia-event-popup">
      <div className="waia-popup-header">
        <b>{cluster.title}</b>
        <button onClick={onClose}>\u00d7</button>
      </div>

      <div className="waia-popup-grid">
        <span>Time:</span>
        <b>{formatClock(cluster.lastSeen)}</b>

        <span>Host:</span>
        <b>{topHost ? `${topHost.hostname}${topHost.ip ? ` (${topHost.ip})` : ''}` : '-'}</b>

        <span>User:</span>
        <b>{cluster.users[0]?.name ?? '-'}</b>

        <span>Source IP:</span>
        <b>{cluster.sourceIps[0]?.name ?? '-'}</b>

        <span>Rule:</span>
        <b>{cluster.ruleIds.slice(0, 2).join(', ') || '-'}</b>
      </div>

      <p>{cluster.explanation}</p>

      <div className="waia-popup-footer">
        <span>Severity:</span>
        <strong style={{ color, borderColor: color }}>{SEVERITY_LABEL[cluster.severity]}</strong>
      </div>
    </div>
  );
}
